...
class EcommerceBundlingSystem:
    ...
    def load_data(self, excel_path):
        ...
        try:
            ...
            print(
                f"Loaded {len(self.orders_df)} orders and {len(self.inventory_df)} inventory items"
            )
    ...
    def preprocess_data(self):
        ...
        # Feature engineering
        df["PriceDiscount"] = (df.get("OriginalUnitPrice", 0) - df.get("FinalUnitPrice", 0)) / (
            df.get("OriginalUnitPrice", 1) + 1e-6
        )
        df["ProfitMargin"] = (df.get("FinalUnitPrice", 0) - df.get("OriginalUnitPrice", 0)) / (
            df.get("FinalUnitPrice", 1) + 1e-6
        )
        ...
        product_stats = (
            df.groupby("SKU")
            .agg({"Quantity": "sum", "OrderNumber": "nunique", "UserID": "nunique"})
            .round(2)
        )
    ...
    def find_complementary_bundles(self):
        ...
        for (sku1, sku2), frequency in frequent_pairs.items():
            ...
            sku1_orders = len(
                self.processed_data[self.processed_data["SKU"] == sku1]["OrderNumber"].unique()
            )
            sku2_orders = len(
                self.processed_data[self.processed_data["SKU"] == sku2]["OrderNumber"].unique()
            )
                ...
                bundles.append(
                    {
                        ...
                        "items": [
                            {
                                "sku": sku1,
                                "title": item1.get("ItemTitle", ""),
                                "category": item1.get("Category", ""),
                                "price": item1.get("FinalUnitPrice", 0),
                            },
                            {
                                "sku": sku2,
                                "title": item2.get("ItemTitle", ""),
                                "category": item2.get("Category", ""),
                                "price": item2.get("FinalUnitPrice", 0),
                            },
                        ],
                        "frequency": frequency,
    ...
    def find_volume_bundles(self):
        ...
        sku_stats = (
            self.processed_data.groupby("SKU")
            .agg(
                {
    ...
    def find_cross_sell_bundles(self):
        ...
        high_margin = product_metrics[
            product_metrics["ProfitMargin"] > product_metrics["ProfitMargin"].quantile(0.7)
        ]

        # Identify popular products
        popular = product_metrics[
            product_metrics["Quantity"] > product_metrics["Quantity"].quantile(0.7)
        ]
        ...
        for margin_sku, margin_row in high_margin.head(10).iterrows():
            for popular_sku, popular_row in popular.head(10).iterrows():
                if margin_sku != popular_sku and margin_row["Category"] != popular_row["Category"]:
                    ...
                    price_balance = 1 - abs(
                        margin_row["FinalUnitPrice"] - popular_row["FinalUnitPrice"]
                    ) / max(margin_row["FinalUnitPrice"], popular_row["FinalUnitPrice"])
    ...
    def visualize_results(self):
        ...
        if self.bundles:
            ...
            axes[0, 1].pie(
                type_counts.values(), labels=type_counts.keys(), autopct="%1.1f%%", startangle=90
            )
    ...
    def generate_report(self, output_file="bundle_analysis_report.txt"):
        ...
        if self.bundles:
            ...
            for btype, stats in bundle_summary.items():
                ...
                report.append(
                    f"{btype} Bundles: {stats['count']} opportunities (avg confidence: {avg_conf:.2f})"
                )
            report.append("")
            ...
            for i, bundle in enumerate(self.bundles[:10], 1):
                report.append(
                    f"{i}. {bundle['type']} Bundle (Confidence: {bundle['confidence']:.2f})"
                )
        ...
        if self.bundles:
            ...
            report.append(
                f"1. Implement {len(high_confidence_bundles)} high-confidence bundles immediately"
            )
            ...
            if comp_bundles:
                report.append(
                    f"2. Focus on Complementary bundles - {len(comp_bundles)} opportunities identified"
                )
            ...
            if cross_sell_bundles:
                report.append(
                    f"4. Use {len(cross_sell_bundles)} Cross-sell bundles to improve margins"
                )
